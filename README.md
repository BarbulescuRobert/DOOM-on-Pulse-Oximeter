...
<img src="./img/img2.jpeg" alt="drawing" height="300"/>
...
Since the OLED looks like the AliExpress modules you can buy cheaply, I started looking at raw displays that fitted the same pattern. I stumbled upon [this]([text](https://www.winstar.com.tw/uploads/files/de98519ca528b9d3bc173cfe8bc99c7d.pdf)) datasheet which looked the same and has the same dimensions, so I went probing around to establish whether it has the same pinout.
...
```c
GPIO_BSRR(GPIOB_BASE) = (1u << OLED_PIN_SCLK);  // ODR[13] = 1, SCLK will idle HIGH
GPIO_BSRR(GPIOB_BASE) = (1u << OLED_PIN_DC);    // ODR[15] = 1, DC will idle HIGH

gpio_output_pushpull(GPIOB_BASE, OLED_PIN_MOSI); // starts LOW
gpio_output_pushpull(GPIOB_BASE, OLED_PIN_SCLK); // starts HIGH (no glitch)
gpio_output_pushpull(GPIOB_BASE, OLED_PIN_DC);   // starts HIGH (no glitch)
```
...
- Hand-written startup assembly with the Thumb-bit fix:
  ...
  Without `.thumb_func`, the linker emits a reset vector with the Thumb bit clear, the Cortex-M0 immediately faults on entry, and J-Link prints the gloriously cryptic `T-bit of XPSR is 0 but should be 1`. If you see that error, this is almost certainly why.
...
- Flashing is done via SEGGER J-Link Commander over SWD:
  ...
  connect
  erase
  loadfile build/pulseox_doom_oled.hex
  r ; g
  ```
...
The OLED bring-up worked fine at the default 8 MHz HSI, but DOOM playback wanted more headroom. I added a short routine in `system_apm32f030.c` that brings the chip up to 48 MHz:
...
- `spi_write_byte` shifts MSB-first, toggling SCLK with one-NOP delays between edges.
- `oled_cmd` drops `D/C#` low, sends one byte, restores `D/C#` high.
- `oled_data` keeps `D/C#` high and streams a buffer.
- `OLED_Update` walks the 8 vertical pages, sending the page-address command (`B0|page`), the column-address commands (`0x00|low`, `0x10|high`), then 128 data bytes per page.
...
The pipeline lives in `tools/video_to_frames.py`. It is a single script that goes from MP4 to a self-contained C file embedded in firmware. It has six stages.
...
```
ffmpeg -i doom_gameplay.mp4 -vf "fps=5,scale=128:64:flags=lanczos" frame_%05d.png
```
...
The SSD1306 stores its framebuffer as 8 horizontal "pages", each 128 bytes wide. One byte = 8 *vertical* pixels in a column. The natural order to emit, page-major, is `page0_col0, page0_col1, ..., page0_col127, page1_col0, ...`.
...
So I emit in **column-major** order instead: `col0_page0..7, col1_page0..7, ..., col127_page0..7`. Now the redundant bytes are adjacent in the stream, which matters enormously for the next stage.
...
The script writes a single self-contained `src/doom_frames.c` containing:

- `frame_data[]` — concatenated RLE bytes for every frame
- `frame_offsets[]` — where each frame starts
- `frame_lengths[]` — how long each frame is
- `DOOM_FRAME_COUNT` and `DOOM_FPS` constants
- A `DoomFrames_Blit(uint32_t idx)` function that decompresses one frame straight into the OLED framebuffer, transposing column-major back to page-major as it goes
...
int main(void)
{
    ...
    GPIO_BSRR(GPIOB_BASE) = (1u << OLED_PIN_PWR);   // PB14 HIGH first!
    gpio_output_pushpull(GPIOB_BASE, OLED_PIN_PWR);

    delay_ms(1000);                                  // OLED rail stabilize
    OLED_Init();
    ...
    while (1) {
        ...
        if (++frame >= DOOM_FRAME_COUNT) frame = 0u;
    }
}
```
